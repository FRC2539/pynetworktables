...
static int ResolveHostName( char* hostname, in_addr* addr)
...
std.unique_ptr<NetworkStream> TCPConnector.connect( char* server,
        int port, logger,
        int timeout)
#ifdef _WIN32
    struct WSAHelper
    ...
    if ((result = .connect(sd, (struct sockaddr*)&address, sizeof(address))) <
            0)
        my_errno = SocketErrno()
#ifdef _WIN32
        if my_errno == WSAEWOULDBLOCK or my_errno == WSAEINPROGRESS:
#else:
        if my_errno == EWOULDBLOCK or my_errno == EINPROGRESS:
#endif
            tv.tv_sec = timeout
            tv.tv_usec = 0
            FD_ZERO(&sdset)
    ...
    # Create stream object if connected, if not.
    if result == -1:
#ifdef _WIN32
        closesocket(sd)
#else:
